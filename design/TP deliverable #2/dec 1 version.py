...
class board(object): #this controls the flow of betting
    ...
    def drawWhosTurn(self, canvas, data): #for displaying whos turn it is
        if data.startingGame == False and data.startHand == False:
    ...
    def find1Pair(self, hand, boardCards):
        cardDict = getCardValDict(self, hand, boardCards)
        pairs = []
    ...
    def getCardValDict(self, hand, boardCards):
        cards = getCardVals(self, hand, boardCards)
        cardDict = {}
        for card in cards:
...
def drawButtons(canvas, data):
    ...
    if data.board.turn == data.user and data.startingGame == False and data.startHand == False:
    ...
    if data.startingGame == True:
        canvas.create_rectangle(data.width/2-data.buttonwidth-10, data.height/2-data.buttonheight-80, data.width/2+data.buttonwidth+10, data.height/2-60,fill="white")
        canvas.create_text(data.width/2, data.height/2-data.buttonheight-45,anchor=CENTER,text="Start Game",font= "Times 30 bold")
    if data.startHand == True:
        canvas.create_rectangle(data.width/2-data.buttonwidth/2-20, data.height/2-data.buttonheight-70, data.width/2+data.buttonwidth/2+20, data.height/2-70,fill="white")
...
def mousePressed(event, data):
    if data.board.turn == data.user and data.startingGame == False:
        if data.width - (data.buttonwidth+data.buttonmargin) < event.x < data.width - data.buttonmargin and (
            ...
            data.user.makeDecision("fold", data.board)
        ...
        if data.width - 3*(data.buttonwidth+data.buttonmargin) < event.x < data.width - 2*(data.buttonwidth+data.buttonmargin)- data.buttonmargin and (
            data.height -data.buttonheight- data.buttonmargin < event.y < data.height - data.buttonmargin):
            data.user.makeDecision("bet", data.board)
    if data.startingGame == True:
        if data.width/2 -data.buttonwidth-10 < event.x < data.width/2+data.buttonwidth+10 and (
            data.height/2 -data.buttonheight-80 < event.y < data.height/2 -60 ):
